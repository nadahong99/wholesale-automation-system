# integrations/wholesalers/murraykorea.py
"""Scraper for Murray Korea (머레이코리아) wholesale site."""
import re
...
class MurraykoreaClient(BaseWholesalerClient):
    wholesaler_name = "murraykorea"
    base_url = "https://www.murraykorea.com"
    login_url = "https://www.murraykorea.com/member/login.html"

    def __init__(self):
        super().__init__(
            username=settings.MURRAYKOREA_USERNAME,
            password=settings.MURRAYKOREA_PASSWORD,
        )
    ...
    def _scrape_products(self, max_products: int = 200) -> List[RawProduct]:
        ...
        while len(products) < max_products:
            ...
            items = soup.select(".xans-record- li.item, .product-item")
            if not items:
                break
            ...
            for item in items:
                ...
                try:
                    ...
                    price_el = item.select_one(".price .sale, .price")
