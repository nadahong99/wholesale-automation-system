# integrations/wholesalers/easymarket.py
"""Scraper for EasyMarket (이지마켓) wholesale site."""
import re
...
class EasymarketClient(BaseWholesalerClient):
    wholesaler_name = "easymarket"
    base_url = "https://www.easyshop.co.kr"
    login_url = "https://www.easyshop.co.kr/member/login.html"

    def __init__(self):
        super().__init__(
            username=settings.EASYMARKET_USERNAME,
            password=settings.EASYMARKET_PASSWORD,
        )
    ...
    def _scrape_products(self, max_products: int = 200) -> List[RawProduct]:
        ...
        while len(products) < max_products:
            ...
            soup = self._get_soup(url, params={"page": page, "cate_no": 1})
            if not soup:
                break
